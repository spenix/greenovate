...
import PageModal from '@/Pages/ViolationType/Modal/IndexModal.vue';
...
const reloadDatatable = () => {
    table.value = $("#violation-type-table").DataTable({
        processing: true,
        serverSide: true,
        ajax: `${page.url}/show_table_data`,
        columns: [
            { data: "name", title: "Violation" },
            { data: "violation_category", title: "Category" },
            {
                data: "id",
                title: "Action",
                orderable: false,
...
const setTableBtnAction = () => {
    $("#violation-type-table").on("click", ".view-btn-option", (e) => {
        e.stopImmediatePropagation();
        var id = $(e.target).data("id");
        btnActionFunc(id, "VIEW");
    });
    $("#violation-type-table").on("click", ".delete-btn-option", (e) => {
        e.stopImmediatePropagation();
        var id = $(e.target).data("id");
        btnActionFunc(id, "DELETE");
    });
    $("#violation-type-table").on("click", ".edit-btn-option", (e) => {
        e.stopImmediatePropagation();
        var id = $(e.target).data("id");
...
const btnActionFunc = (id, action) => {
    ...
    modalAttrs.value.title = `${action} VIOLATION TYPE`;
                ...
                case "catch":
                    router.delete(`${page?.url}/destroy/${id}`, {
                        onSuccess: () => {
                            swal(
                                "Success!",
                                "Violation type was deleted successfully.",
                                "success"
                            ),
                            reloadDatatableAjax();
                        },
...
<template>
    ...
    <main id="main" class="main">
        ...
        <nav>
            ...
            <li class="breadcrumb-item active">Violation Type</li>
            </ol>
        </nav>
        </div>
        <section class="section">
            <div class="card">
              <div class="card-body">
                <h5 class="card-title">Violation Type List</h5>
                <div class="row">
                    <div class="col-12 p-2 text-end">
                        <button type="button" class="btn btn-primary btn-sm" @click="btnActionFunc('', 'ADD')">Add Violation Type</button>
                    </div>
                    <div class="col-12 p-2">
                        <div class="table-responsive">
                            <table
                                class="table table-bordered table-sm display"
                                id="violation-type-table"
