...
import PageModal from '@/Pages/EmployeeLeave/Modal/IndexModal.vue';
...
const reloadDatatable = () => {
    table.value = $("#employee-leave-table").DataTable({
        processing: true,
        serverSide: true,
        ...
        columns: [
            ...
            { 
                ...
                render(h, type, row) {
                    return `${moment(row?.date_start).format('L')}-${moment(row?.date_end).format('L')}`;
                }, 
            },
...
const setTableBtnAction = () => {
    $("#employee-leave-table").on("click", ".view-btn-option", (e) => {
        e.stopImmediatePropagation();
        var id = $(e.target).data("id");
        btnActionFunc(id, "VIEW");
    });
    $("#employee-leave-table").on("click", ".delete-btn-option", (e) => {
        e.stopImmediatePropagation();
        var id = $(e.target).data("id");
        btnActionFunc(id, "DELETE");
    });
    $("#employee-leave-table").on("click", ".edit-btn-option", (e) => {
        e.stopImmediatePropagation();
        var id = $(e.target).data("id");
...
const btnActionFunc = (id, action) => {
    ...
    modalAttrs.value.title = `${action} EMPLOYEE LEAVE`;
                ...
                case "catch":
                    router.delete(`${page?.url}/destroy/${id}`, {
                        onSuccess: () => {
                            swal(
                                "Success!",
                                "Employee leave was deleted successfully.",
                                "success"
                            ),
                            reloadDatatableAjax();
                        },
...
<template>
    ...
    <main id="main" class="main">
        ...
        <nav>
            ...
            <li class="breadcrumb-item active">Employee Leave Credits</li>
            </ol>
        </nav>
        </div>
